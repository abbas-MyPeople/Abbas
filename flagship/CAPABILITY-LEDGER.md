...
| 17 | **Unified customer identity (CDP) + receipt-scan 3P→direct conversion** (unify every customer across all voice/order channels into one profile; analyze their nature/behavior; act on it — targeted marketing, free gifts; and **convert a third-party-app customer into a direct, known one on the spot: scan the order receipt → instant free offer → captured with every order**) | New project in active build at the flagship. Builds on the voice agent's returning-caller memory (#6), the direct-conversion tactic (#4), and analytics (#3). | 🔵 (in build) | — | 🔴 | **High-value future SKU** — a real CDP for independents plus a point-of-receipt 3P→direct conversion loop (captures the app's own customer at every order, not just via the bag insert). Strengthens both "Keep the guest" and "Commission Rescue." Sell the *direction* now; no delivered result yet. |
